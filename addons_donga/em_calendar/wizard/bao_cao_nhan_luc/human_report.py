...
class HumanReport(models.TransientModel):
    ...
    def _get_data_khoa_kham_benh(self):
        ...
        inpatient_moi = self.env['sh.medical.evaluation'].search([('evaluation_start_date', '>=', self.start_date), ('evaluation_start_date', '<=', self.end_date), ('ward', '=', self.env.ref('shealth_all_in_one.sh_kb_dep_knhn').id)])
        ...
        inpatient_xuat = self.env['sh.medical.evaluation'].search(
            [('evaluation_end_date', '>=', self.start_date), ('evaluation_end_date', '<=', self.end_date), ('ward', '=', self.env.ref('shealth_all_in_one.sh_kb_dep_knhn').id)])

        """ Đứng ở Thông tin bác sĩ tìm ra tất cả bản ghi """
        print('hello')
        physician = self.env['sh.medical.physician'].search([('department', '=', self.env.ref('shealth_all_in_one.sh_kb_dep_knhn').id), ('is_resign', '=', False)])
    ...
    def _get_data_khoa_gay_me_hoi_suc(self):
        ...
        inpatient_cu = self.env['sh.medical.surgery'].search([('surgery_date', '<', self.start_date), ('state', 'in', ['Confirmed', 'In Progress']), ('department', '=', self.env.ref('shealth_all_in_one.sh_surgeries_dep_knhn').id)])


        """ Đứng ở phiếu PTTT tìm ra tất cả bản ghi có trạng thái đang nhập viện, lấy ra bệnh nhân mới """
        inpatient_moi = self.env['sh.medical.surgery'].search([('surgery_date', '>=', self.start_date), ('surgery_date', '<=', self.end_date), ('department', '=', self.env.ref('shealth_all_in_one.sh_surgeries_dep_knhn').id)])
        ...
        inpatient_xuat = self.env['sh.medical.surgery'].search(
            [('surgery_end_date', '>=', self.start_date), ('surgery_end_date', '<=', self.end_date), ('department', '=', self.env.ref('shealth_all_in_one.sh_surgeries_dep_knhn').id)])

        """ Đứng ở Thông tin bác sĩ tìm ra tất cả bản ghi """
        physician = self.env['sh.medical.physician'].search([('department', '=', self.env.ref('shealth_all_in_one.sh_surgeries_dep_knhn').id), ('is_resign', '=', False)])
    ...
    def _get_data_khoa_pttm(self):
        ret_data = []
        """ Đứng ở bệnh nhân lưu tìm ra tất cả bản ghi có trạng thái đang nhập viện, lấy ra bệnh nhân cũ """
        inpatient_cu = self.env['sh.medical.inpatient'].search([('admission_date', '<', self.start_date), ('state', '=', 'Hospitalized'), ('ward', '=', 6)])

        """ Đứng ở bệnh nhân lưu tìm ra tất cả bản ghi có trạng thái đang nhập viện, lấy ra bệnh nhân mới """
        inpatient_moi = self.env['sh.medical.inpatient'].search([('admission_date', '>=', self.start_date), ('admission_date', '<=', self.end_date), ('ward', '=', 6)])

        """ Đứng ở bệnh nhân lưu tìm ra tất cả bản ghi có trạng thái đang nhập viện, đã nhập viện, lấy ra bệnh nhân xuất viện """
        inpatient_xuat = self.env['sh.medical.inpatient'].search(
            [('discharge_date', '>=', self.start_date), ('discharge_date', '<=', self.end_date), ('ward', '=', 6)])

        """ Đứng ở Thông tin bác sĩ tìm ra tất cả bản ghi """
        physician = self.env['sh.medical.physician'].search([('department', '=', 6), ('is_resign', '=', False)])
    ...
    def _get_data_khoa_rang_ham_mat(self):
        ...
        """ Đứng ở bệnh nhân lưu tìm ra tất cả bản ghi có trạng thái đang nhập viện, lấy ra bệnh nhân mới """
        inpatient_moi = self.env['sh.medical.specialty'].search([('services_date', '>=', self.start_date), ('services_date', '<=', self.end_date), ('department', '=', self.env.ref('shealth_all_in_one.sh_rhm_dep_knhn').id)])

        """ Đứng ở bệnh nhân lưu tìm ra tất cả bản ghi có trạng thái đang nhập viện, đã nhập viện, lấy ra bệnh nhân xuất viện """
        inpatient_xuat = self.env['sh.medical.specialty'].search(
            [('services_end_date', '>=', self.start_date), ('services_end_date', '<=', self.end_date), ('department', '=', self.env.ref('shealth_all_in_one.sh_rhm_dep_knhn').id)])

        """ Đứng ở Thông tin bác sĩ tìm ra tất cả bản ghi """
        physician = self.env['sh.medical.physician'].search([('department', '=', self.env.ref('shealth_all_in_one.sh_rhm_dep_knhn').id), ('is_resign', '=', False)])
    ...
    def _get_data_khoa_da_lieu(self):
        ...
        """ Đứng ở bệnh nhân lưu tìm ra tất cả bản ghi có trạng thái đang nhập viện, lấy ra bệnh nhân mới """
        inpatient_moi = self.env['sh.medical.specialty'].search([('services_date', '>=', self.start_date), ('services_date', '<=', self.end_date), ('department', '=', self.env.ref('shealth_all_in_one.sh_dalieu_dep_knhn').id)])

        """ Đứng ở bệnh nhân lưu tìm ra tất cả bản ghi có trạng thái đang nhập viện, đã nhập viện, lấy ra bệnh nhân xuất viện """
        inpatient_xuat = self.env['sh.medical.specialty'].search(
            [('services_end_date', '>=', self.start_date), ('services_end_date', '<=', self.end_date), ('department', '=', self.env.ref('shealth_all_in_one.sh_dalieu_dep_knhn').id)])

        """ Đứng ở Thông tin bác sĩ tìm ra tất cả bản ghi """
        physician = self.env['sh.medical.physician'].search([('department', '=', self.env.ref('shealth_all_in_one.sh_dalieu_dep_knhn').id), ('is_resign', '=', False)])
    ...
    def _get_data_khoa_can_lam_san(self):
        ret_data = []
        # """ Đứng ở bệnh nhân lưu tìm ra tất cả bản ghi có trạng thái đang nhập viện, lấy ra bệnh nhân cũ """
        # inpatient_cu = self.env['sh.medical.lab.test'].search([('admission_date', '<', self.start_date), ('state', '=', 'Hospitalized'), ('ward', '=', self.env.ref('shealth_all_in_one.sh_labtest_dep_knhn').id)])

        """ Đứng ở bệnh nhân lưu tìm ra tất cả bản ghi có trạng thái đang nhập viện, lấy ra bệnh nhân mới """
        inpatient_moi = self.env['sh.medical.lab.test'].search([('date_analysis', '>=', self.start_date), ('date_analysis', '<=', self.end_date), ('department', '=', self.env.ref('shealth_all_in_one.sh_labtest_dep_knhn').id)])

        """ Đứng ở bệnh nhân lưu tìm ra tất cả bản ghi có trạng thái đang nhập viện, đã nhập viện, lấy ra bệnh nhân xuất viện """
        inpatient_xuat = self.env['sh.medical.lab.test'].search(
            [('date_done', '>=', self.start_date), ('date_done', '<=', self.end_date), ('department', '=', self.env.ref('shealth_all_in_one.sh_labtest_dep_knhn').id)])

        """ Đứng ở Thông tin bác sĩ tìm ra tất cả bản ghi """
        physician = self.env['sh.medical.physician'].search([('department', '=', self.env.ref('shealth_all_in_one.sh_labtest_dep_knhn').id), ('is_resign', '=', False)])
