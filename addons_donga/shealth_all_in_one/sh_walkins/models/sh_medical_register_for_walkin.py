...
class SHealthAppointmentWalkin(models.Model):
    ...
    name = fields.Char(string='Queue #', size=128, required=True, default=lambda *a: '/', states={'Completed': [('readonly', True)]}, track_visibility='always')
    patient = fields.Many2one('sh.medical.patient', string='Patient', help="Patient Name", required=True, readonly=False, states={'Completed': [('readonly', True)]})
    ...
    marital_status = fields.Selection(MARITAL_STATUS, string='Marital Status', related="patient.marital_status", store=True, track_visibility='onchange')
    blood_type = fields.Selection(BLOOD_TYPE, string='Blood Type', related="patient.blood_type", store=True, track_visibility='onchange')
    rh = fields.Selection(RH, string='Rh', readonly=False, related="patient.rh", store=True, track_visibility='onchange')
    doctor = fields.Many2one('sh.medical.physician', string='Responsible Physician', readonly=False, states={'Completed': [('readonly', True)]},domain=lambda self:self.get_physician_kb(),default=_get_physician, track_visibility='onchange')
    state = fields.Selection(WALKIN_STATUS, string='State', readonly=False, states={'Completed': [('readonly', True)]}, default=lambda *a: 'Scheduled', track_visibility='onchange')
    comments = fields.Text(string='Comments', readonly=False, states={'Completed': [('readonly', True)]}, track_visibility='onchange')
    ...
    evaluation_ids = fields.One2many('sh.medical.evaluation', 'walkin', string='Evaluation', readonly=False, states={'Completed': [('readonly', True)]})
    prescription_ids = fields.One2many('sh.medical.prescription','walkin', string='Prescriptions', readonly=False, states={'Completed': [('readonly', True)]})
    lab_test_ids = fields.One2many('sh.medical.lab.test', 'walkin', string='Lab Tests', readonly=False, states={'Completed': [('readonly', True)]})
    imaging_ids = fields.One2many('sh.medical.imaging', 'walkin', string='Imaging Tests', readonly=False, states={'Completed': [('readonly', True)]})
    surgeries_ids = fields.One2many('sh.medical.surgery', 'walkin', string='Surgeries Tests', readonly=False, states={'Completed': [('readonly', True)]})
    specialty_ids = fields.One2many('sh.medical.specialty', 'walkin', string='Specialty Tests', readonly=False, states={'Completed': [('readonly', True)]})
    inpatient_ids = fields.One2many('sh.medical.inpatient', 'walkin', string='Inpatient Admissions', readonly=False, states={'Completed': [('readonly', True)]})
    # vaccine_ids = fields.One2many('sh.medical.vaccines', 'walkin', string='Vaccines', readonly=False, states={'Completed': [('readonly', True)]})

    institution = fields.Many2one('sh.medical.health.center', string='Health Center', required=True, readonly=False, states={'Completed': [('readonly', True)]}, track_visibility='onchange')
    department = fields.Many2one('sh.medical.health.center.ward', string='Department',
                                 help="Department of the selected Health Center",
                                 domain="[('institution','=',institution)]", readonly=False, states={'Completed': [('readonly', True)]}, track_visibility='onchange')
    ...
    reason_check = fields.Text('Reason Check', related="reception_id.reason", readonly=False, states={'Completed': [('readonly', True)]}, track_visibility='onchange')
    ...
    service = fields.Many2many('sh.medical.health.center.service', 'sh_walkin_service_rel','walkin_id','service_id', readonly=False, states={'Completed': [('readonly', True)]}, track_visibility='onchange',string='Services')
    service_room = fields.Many2one('sh.medical.health.center.ot', string='Room perform',
                                   domain="[('department', '=', department)]", readonly=False, states={'Completed': [('readonly', True)]}, track_visibility='onchange')
    service_expected = fields.Many2many('sh.medical.health.center.service', 'sh_walkin_service_expected_rel','walkin_id','service_id', readonly=True, track_visibility='onchange',string='Services Expected')
    ...
    date_re_exam = fields.Datetime(string='Date Re-exam', readonly=False, states={'Completed': [('readonly', True)]}, track_visibility='onchange')
    date_out = fields.Datetime(string='Ngày ra viện', readonly=False, states={'Completed': [('readonly', True)]}, track_visibility='onchange')
    ...
    #phiếu khám gốc
    root_walkin = fields.Many2one('sh.medical.appointment.register.walkin', string='Root walkin', domain="[('patient','=',patient)]", hint="Dùng liên kết các phiếu khám của dịch vụ liệu trình hoặc tái khám", states={'Completed': [('readonly', True)]}, track_visibility='onchange')
    ...
    doctor_surgeon = fields.Many2one('sh.medical.physician', string='Bác sĩ phẫu thuật', compute="_compute_doctor_surgeon")
    doctor_anesthetist = fields.Many2one('sh.medical.physician', string='Bác sĩ gây mê', compute="_compute_doctor_surgeon")
    ...
    physical_exam = fields.Text('Toàn thân',default="""-	Thể trạng khá.\n-	Da không xanh , niêm mạc hồng.\n-	Tuyến giáp không to.\n-	Hạnh ngoại biên không sờ thấy.""")
    ...
    def _compute_date_to_index(self):
        for record in self:
            ...
            data = self.env['sh.medical.appointment.register.walkin'].search([('date','>=',start),('date','<=',end),('service_room','=',record.service_room.id)],order="date")
            # print(self.id)
            if len(data)>0:
        ...
        for record in self:
            ...
            if record.service:
                record.compute_service = self.env.ref('shealth_all_in_one.sh_product_service_kb01').product_id + \
                    record.lab_test_ids.filtered(lambda m: m.state in ['Test In Progress', 'Completed']).\
                          mapped('test_type.product_id') + \
                    record.imaging_ids.filtered(lambda m: m.state in ['Test In Progress', 'Completed']). \
                          mapped('test_type.product_id') + \
    ...
    def update_walkin_material(self):
        ...
        for lab in self.lab_test_ids.filtered(lambda l: l.state == 'Completed'):
            for lab_mats in lab.lab_test_material_ids:
                ...
                lab_dict_key = str(lab_mats.product_id.id) + '-' + str(lab.department.id) + '-' + 'Labtest' + '-' + str(lab.perform_room.location_supply_stock.id)
                lab_qty_line = lab_mats.uom_id._compute_quantity(lab_mats.quantity, lab_mats.product_id.uom_id)
                lab_init_qty_line = lab_mats.uom_id._compute_quantity(lab_mats.init_quantity, lab_mats.product_id.uom_id)
                ...
                if mats[id_marterial_lab[lab_dict_key] - 1][2]['quantity'] > mats[id_marterial_lab[lab_dict_key] - 1][2]['init_quantity']:
                    over = True
        ...
        for inpatient in self.inpatient_ids:
            ...
            for rounding in inpatient.roundings:
                ...
                for rounding_mat in rounding.medicaments:
                    ...
                    rounding_qty_line = rounding_mat.uom_id._compute_quantity(rounding_mat.qty, rounding_mat.medicine.uom_id)
            ...
            for medicine in prescription.prescription_line:
                ...
                if medicine.name.id not in id_medicine_line:
                    ...
                    mats.append((0, 0, {'product_id': medicine.name.id,
                                        ...
                                        'institution': self.institution.id,
                                        'department': self.service[0].service_department.filtered(lambda s: s.institution == self.institution)[0].id if self.service[0].service_department else 1,
    ...
    def set_to_completed(self):
        ...
        if len(self.imaging_ids.filtered(lambda lt: lt.state not in ["Completed"])) > 0:
            raise UserError(_('Bạn không thể đóng phiếu khi có phiếu Chuẩn đoan hình ảnh - Thăm dò chức năng được xác nhận hoàn thành!'))
        # còn phiếu pttt
        if len(self.surgeries_ids.filtered(lambda lt: lt.state not in ["Done","Signed"])) > 0:
            raise UserError(_('Bạn không thể đóng phiếu khi có phiếu Phẫu thuật thủ thuật chưa được xác nhận hoàn thành!'))
    ...
    def set_to_progress(self):
            ...
            for lab in ser.lab_type_ids:
                if lab.id not in id_lab_test_wakin:
                    ...
                    lab_test_wakin.append((0, 0, {
                                               ...
                                               'test_type': lab.id,
                                               'perform_room': False if self.env.ref('shealth_all_in_one.sh_labtest_room_knhn', False) == None else self.env.ref('shealth_all_in_one.sh_labtest_room_knhn').id,
                                               'has_child': lab.has_child,
                                               'normal_range': lab.normal_range,
                                               'patient': self.patient.id,
                                               'pathologist': self.env.ref('__import__.data_physician_2').id if self.env.ref('__import__.data_physician_2',False) else False,
                                               'date_requested': datetime.datetime.strptime(self.date.strftime("%Y-%m-%d %H:%M:%S"), "%Y-%m-%d %H:%M:%S") + timedelta(minutes=15) or fields.Datetime.now(),
                                               'date_analysis': datetime.datetime.strptime(self.date.strftime("%Y-%m-%d %H:%M:%S"), "%Y-%m-%d %H:%M:%S") + timedelta(minutes=30) or fields.Datetime.now(),
                                               'date_done': datetime.datetime.strptime(self.date.strftime("%Y-%m-%d %H:%M:%S"), "%Y-%m-%d %H:%M:%S") + timedelta(hours=2) or fields.Datetime.now(),
                                               'requestor':self.doctor.id,
            ...
            for img in ser.imaging_type_ids:
                if img.id not in id_img_test_wakin:
                    ...
                    img_test_wakin.append((0, 0, {
                                              ...
                                              'patient': self.patient.id,
                                              'pathologist': self.env.ref('__import__.data_physician_canlamsang').id if self.env.ref('__import__.data_physician_canlamsang',False) else False,
                                              'date_requested': datetime.datetime.strptime(self.date.strftime("%Y-%m-%d %H:%M:%S"), "%Y-%m-%d %H:%M:%S") + timedelta(minutes=15) or fields.Datetime.now(),
                                              'date_analysis': datetime.datetime.strptime(self.date.strftime("%Y-%m-%d %H:%M:%S"), "%Y-%m-%d %H:%M:%S") + timedelta(minutes=30) or fields.Datetime.now(),
                                              'date_done': datetime.datetime.strptime(self.date.strftime("%Y-%m-%d %H:%M:%S"), "%Y-%m-%d %H:%M:%S") + timedelta(hours=2) or fields.Datetime.now(),
                                              'requestor': self.doctor.id,
        ...
        if self.flag_surgery:
            sur_department = self.env['sh.medical.health.center.ward'].search([('institution','=',self.institution.id),('type','=','Surgery')],limit=1)
            ...
            room_minor = False if self.env.ref('shealth_all_in_one.sh_supersonic_tieuphau_room_knhn', False) == None else self.env.ref('shealth_all_in_one.sh_supersonic_tieuphau_room_knhn').id
            room_major = False if self.env.ref('shealth_all_in_one.sh_supersonic_daiphau_room_knhn', False) == None else self.env.ref('shealth_all_in_one.sh_supersonic_daiphau_room_knhn').id
            ...
            surgery_wakin.append((0, 0, {
                  ...
                  'surgeon': self.env.ref('__import__.data_physician_pttm').id if self.env.ref('__import__.data_physician_pttm',False) else False,
                  'anesthetist': self.env.ref('__import__.data_physician_gmhs').id if self.env.ref('__import__.data_physician_gmhs',False) else False,
                  'walkin': self.id,
                  'state': 'Draft'}))
        else:
            spec_department = self.sudo().service[0].service_department.filtered(lambda s: s.institution == self.institution)[0].id if self.sudo().service[0].service_department else False
            spec_room = self.sudo().service[0].service_room.filtered(lambda s: s.institution == self.institution)[0].id if self.sudo().service[0].service_room else False

            if spec_department and self.sudo().service[0].service_department.filtered(lambda s: s.institution == self.institution)[0].get_external_id() == 'shealth_all_in_onesh_dalieu_dep_knhn':
                physician_ck = self.env.ref('__import__.data_physician_dalieu').id if self.env.ref('__import__.data_physician_dalieu',False) else False
            else:
                physician_ck = self.env.ref('__import__.data_physician_rhm').id if self.env.ref('__import__.data_physician_rhm',False) else False
        ...
        return self.sudo().write({'state': 'InProgress','lab_test_ids': lab_test_wakin,'imaging_ids': img_test_wakin,'surgeries_ids': surgery_wakin,'specialty_ids':specialty_wakin})
...
class SHealthSurgery(models.Model):
    ...
    _order = "walkin"
    walkin = fields.Many2one('sh.medical.appointment.register.walkin', string='Queue #', required=True, readonly=True, ondelete='cascade')
