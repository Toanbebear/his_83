...
class InheritSHealthPrescriptions(models.Model):
    ...
    def action_sync_his_prescription(self):
        ...
        for path in self.walkin.pathology:
            ket_luan = self.walkin.service.mapped('name')
            ket_luan_filtered = [value for value in ket_luan if value is not False]

            ten_chuan_doan = self.walkin.service.mapped('diagnose')
            ten_chuan_doan_filtered = [value for value in ten_chuan_doan if value is not False]

            # Thay đổi cấu trúc dữ liệu
            thong_tin_chan_doan = {
                "ma_chan_doan": path.code if path.code else '',
                "ten_chan_doan": ', '.join([str(value) for value in ten_chuan_doan_filtered]),
        ...
        if self.doctor.token_his_physician:
            ...
            payload = json.dumps({
                ...
                "ngay_tai_kham": so_ngay if so_ngay else '',
                "ngay_gio_ke_don": ngay_gio_ke_don.strftime('%Y-%m-%d %H:%M:%S'),
                "signature": '',
            })
            # _logger.info("-------------------------------------------------------------------------",  payload.encode('utf-8').decode('unicode-escape'))
            headers = {
            ...
            if 'status' in response and response['status'] == 'success':
                time.sleep(3)
                url_sale = 'http://10.10.10.197:9988/api/eprescription/sale?api=2b11k2h3foes9f0809zdn398f0fasdmkj30'
