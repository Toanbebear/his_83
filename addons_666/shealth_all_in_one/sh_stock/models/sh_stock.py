...
class SHealthStockPicking(models.Model):
    ...
    picking_in = fields.Boolean('Phiếu nhập', default=False)
    ...
    def _get_check_stock(self):
        self.ensure_one()
        return self.move_lines.filtered(lambda rec: rec.check_quantity == True)
    ...
    def button_validate(self):
        ...
        if self.move_ids_without_package and self.picking_in == False:
            for rec in self.move_ids_without_package:
                if rec.check_quantity == True:
                    view = self.env.ref('shealth_all_in_one.view_overprocessed_transfer_inherit')
                    wiz = self.env['inherit.stock.overprocessed.transfer'].create({'picking_id': self.id})
                    return {
                        'type': 'ir.actions.act_window',
                        'view_type': 'form',
                        'view_mode': 'form',
                        'res_model': 'inherit.stock.overprocessed.transfer',
                        'views': [(view.id, 'form')],
                        'view_id': view.id,
                        'target': 'new',
                        'res_id': wiz.id,
        ...
        if self.env.context.get('view_for', True) == 'picking_int_return':
            for rec in self:
                if rec.location_id.location_institution_type == 'medicine':
                    rec.location_dest_id = self.env.ref('shealth_all_in_one.sh_location_medicine_prescription_knhn').id
    ...
    def compute_check_quantity(self):
        for rec in self:
            ...
            for record in quantity:
                if record.qty_done > record.check_ton:
...
class SHealthStockMoveLine(models.Model):
    ...
    check_ton = fields.Float('Check tồn kho', compute='compute_check_ton', store=True)

    @api.depends('location_id')
    def compute_check_ton(self):
        for record in self:
            record.check_ton = False
            if record.lot_id:
                location_id = record.lot_id.quant_ids
                ton_kho = location_id.filtered(lambda rec: rec.location_id.id == record.location_id.id)
                if ton_kho:
                    record.check_ton = ton_kho.quantity
            else:
                ton_kho = self.env['stock.quant'].sudo().search([('location_id', '=', record.location_id.id), ('product_id', '=', self.product_id.id)], limit=1)
                record.check_ton = ton_kho[0].quantity if ton_kho else None
...
class SHealthStockScrap(models.Model):
    ...
    # Hoàn trả tiêu hủy thuốc vật tư
        ...
        for rec in self:
            return {
                ...
                'context': {
                    ...
                    'default_origin': 'Hoàn lại phiếu' + ' ' + rec.name,
    ...
    #     Hoàn trả xuất sử dụng phòng
    def action_return_validate_sdp(self):
        for rec in self:
            return {
                'name': 'Hoàn xuất sử dụng phòng',
                'view_mode': 'form',
                'res_model': 'return.stock.scrap.sdp',
                'type': 'ir.actions.act_window',
                'view_id': self.env.ref('shealth_all_in_one.return_stock_scrap_sdp_form').id,
                'context': {
                    'default_scrap_id': rec.id,
                    'default_product_id': rec.product_id.id,
                    'default_room_use': rec.room_use.id,
                    'default_location_id': rec.scrap_location_id.id,
                    'default_product_uom_id': rec.product_uom_id.id,
                    'default_location_dest_id': rec.location_id.id,
                    'default_scrap_qty': rec.scrap_qty,
    ...
    @api.onchange('location_id')
    def onchange_location_id(self):
        if self.location_id != self.product_id:
...
class SHealthPurchaseOrder(models.Model):
    ...
    def button_confirm(self):
        ...
        for purchase in self:
            ...
            # check laf phiếu nhập
            picking_related.picking_in = True
