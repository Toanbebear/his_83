...
class SHealthPharmacy(models.Model):
    _name = 'sh.medical.health.center.pharmacy'
    ...
    # pharmacist_name = fields.Many2one('sh.medical.physician', string='Pharmacist Name', domain=[('is_pharmacist','=',True)], required=True)
    institution = fields.Many2one('sh.medical.health.center', string='Health Center')
    location_id = fields.Many2one('stock.location', string='Stock Location')
    pharmacist_ids = fields.One2many('sh.medical.health.center.pharmacy.pharmacist', 'pharmacy_id', string='Pharmacist')
    pharmacy_lines = fields.One2many('sh.medical.health.center.pharmacy.line', 'pharmacy_id', string='Pharmacy Lines')
    ...
    def create(self, vals):
        ...
        pharmacy = super(SHealthPharmacy, self).create(vals)
        return pharmacy
...
class SHealthPharmacist(models.Model):
    _name = 'sh.medical.health.center.pharmacy.pharmacist'
    _description = 'Pharmacist of Pharmacy'

    pharmacy_id = fields.Many2one('sh.medical.health.center.pharmacy', string='Pharmacy')
    pharmacist_id = fields.Many2one('sh.medical.physician', string='Pharmacist', domain=[('is_pharmacist', '=', True)])
    institution = fields.Many2one('sh.medical.health.center', string='Institution', help="Institution where pharmacist works")

class SHealthPharmacyLines(models.Model):
    _name = 'sh.medical.health.center.pharmacy.line'
    ...
    name = fields.Many2one('sh.medical.prescription', string='Prescription #', required=True, ondelete='cascade', readonly=True, states={'Draft': [('readonly', False)]})
    ...
    doctor = fields.Many2one('sh.medical.physician', string='Physician', help="Current primary care / family doctor", domain=[('is_pharmacist','=',False)], readonly=True, states={'Draft': [('readonly', False)]})
    prescription_lines = fields.One2many('sh.medical.health.center.pharmacy.prescription.line', 'prescription_id', string='Prescription Lines', readonly=True, states={'Draft': [('readonly', False)]})
    pharmacy_id = fields.Many2one('sh.medical.health.center.pharmacy', string='Pharmacy Reference', readonly=True, states={'Draft': [('readonly', False)]})
    ...
    def _onchange_prescription_id_values(self,prescription_id):
        phar_pres_line_obj = self.env['sh.medical.health.center.pharmacy.prescription.line']
        pres_obj = self.env['sh.medical.prescription']
        prescription_ids =[]
        res = {}
        ...
        query = _("select name, indication, qty from sh_medical_prescription_line where prescription_id = %s")%(str(prescription_id))
        ...
        if vals:
            for va in vals:
                ...
                query1 = _("select pt.list_price from sh_medical_medicines om, product_product po, product_template pt where po.product_tmpl_id=pt.id and om.product_id=po.id and om.id = %s")%(str(va[0]))
    ...
    def unlink(self):
        ...
        return super(SHealthPharmacyLines, self).unlink()

class SHealthPharmacyMedicineLines(models.Model):
    _name = 'sh.medical.health.center.pharmacy.prescription.line'
    ...
    prescription_id = fields.Many2one('sh.medical.health.center.pharmacy.line', string='Pharmacy Prescription Reference')
    ...
    def onchange_medicine_id(self):
        result = {}
        if self.name:
            med_obj = self.env['sh.medical.medicines']
