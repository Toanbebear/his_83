...
class SHealthStockMove(models.Model):
    ...
    def compute_check_quantity(self):
        for rec in self:
            for record in rec.move_line_ids:
                if record.location_id.id != 8 and record.qty_done > record.check_ton:
                    print('checlton',record.check_ton)
...
class SHealthPurchaseOrder(models.Model):
    ...
    _sql_constraints = [
        ('name_partner_uniq', 'unique (name,partner_id)', 'Mã đơn hàng của nhà cung cấp này đã tồn tại trong hệ thống')]
    ...
    def view_choose_stock_picking(self):
        return {
            'name': _('GẮN VỚI PHIẾU ĐIỀU CHUYỂN'),  # label
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'view_id': self.env.ref('shealth_all_in_one.form_view_choose_stock_picking_wizard').id,
            'res_model': 'choose.stock.picking',  # model want to display
