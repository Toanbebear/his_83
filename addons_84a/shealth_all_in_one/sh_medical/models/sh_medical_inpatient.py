...
class SHealthInpatient(models.Model):
    ...
    def _get_ward(self):
        """Return default physician value"""
        wards = self.env['sh.medical.health.center.ward']
        domain = [('type', '=', 'Inpatient')]
        ward_ids = wards.search(domain, limit=1)
        if ward_ids:
            return ward_ids.id or False
        else:
            return False
    ...
    admission_reason = fields.Many2many('sh.medical.pathology', string='Reason for Admission', help="Reason for Admission", readonly=True, states={'Draft': [('readonly', False)]})
    ...
    ward = fields.Many2one('sh.medical.health.center.ward', string='Ward', required=True, readonly=True, states={'Draft': [('readonly', False)]},default=_get_ward)
    ...
    def write(self, vals):
        if vals.get('admission_date') or vals.get('discharge_date'):
            for record in self:
                ...
                # format to date
                if isinstance(admission_date, pycompat.string_types):
                    admission_date = datetime.datetime.strptime(admission_date, '%Y-%m-%d %H:%M:%S')
                if isinstance(discharge_date, pycompat.string_types):
    ...
    def create(self, vals):
        sequence = self.env['ir.sequence'].next_by_code('sh.medical.inpatient')
