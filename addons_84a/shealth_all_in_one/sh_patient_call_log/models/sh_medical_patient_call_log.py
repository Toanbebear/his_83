...
class SHealthPatientCallLog(models.Model):
    _name = 'sh.medical.patient.call.log'
    ...
    def create(self, vals):
        sequence = self.env['ir.sequence'].next_by_code('sh.medical.patient.call.log')
        vals['name'] = sequence
        return super(SHealthPatientCallLog, self).create(vals)
    ...
    def write(self, vals):
        ...
        return super(SHealthPatientCallLog, self).write(vals)

    # Inheriting Patient module to add "Call Logs" screen reference
class SHealthPatient(models.Model):
    _inherit='sh.medical.patient'
    call_log_ids = fields.One2many('sh.medical.patient.call.log', 'patient', string='Call Logs')
