    ...
    def _get_inventory_data(self):
        ...
        for product in product_list:
            ...
            if product_lot_list:
                ...
                product_group_line = [product.default_code, product.name, product.uom_id.name, 'group'] + [''] * 6 + [
                    0] * (19 + len(locations) * 7) # group lines by product
                ...
                for lot in product_lot_list:
                    ...
                    prod = self.env['sh.medical.medicines'].search([('product_id', '=', product.id)], limit=1)
                    product_lot_line = [product.default_code or '', product.name, product.uom_id.name, prod.registration_code or '', prod.production_company or '', prod.origin.name or '', lot.name or '', lot.removal_date or '', lot_unit_price, lot_unit_price*1.05]
                    ...
                    product_lot_line.extend([qty_begin_total, qty_begin_total * lot_unit_price, qty_in_total, qty_in_total * lot_unit_price,
                                            qty_out_patient_total, qty_out_scrap_total, qty_out_room_use_total, qty_out_supplier_total,
                                            (qty_out_patient_total + qty_out_scrap_total + qty_out_room_use_total + qty_out_supplier_total) * lot_unit_price,
                                            qty_end_total, qty_end_total * lot_unit_price])
                    # main location data
                    ...
                    for i in range(10, 29 + len(locations) * 7):
