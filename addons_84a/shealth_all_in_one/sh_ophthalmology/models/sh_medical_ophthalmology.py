...
class SHealthOphthalmology(models.Model):
    _name = "sh.medical.ophthalmology"
    ...
    def _patient_age_at_evaluation(self):
        def compute_age_from_dates (patient_dob,patient_visit_date):
            if (patient_dob):
                ...
                years_months_days = _("%s years %s days"%(str(delta.days // 365),str(delta.days%365)))
            else:
    ...
    doctor = fields.Many2one('sh.medical.physician','Physician', domain=[('is_pharmacist','=',False)], help="Health professional / Ophthalmologist / OptoMetrist", required=True, readonly=True, states={'Draft': [('readonly', False)]}, default=_get_physician)
    rdva = fields.Selection(SNELL_CHART, string='RDVA (Without Aid)', help="Right Eye Vision of Patient without aid", readonly=True, states={'In Progress': [('readonly', False)]})
    ldva = fields.Selection(SNELL_CHART, string='LDVA (Without Aid)', help="Left Eye Vision of Patient without aid", readonly=True, states={'In Progress': [('readonly', False)]})
    rdva_pinhole = fields.Selection(SNELL_CHART, string='RDVA (Pin Hole)', help="Right Eye Vision Using Pin Hole", readonly=True, states={'In Progress': [('readonly', False)]})
    ldva_pinhole = fields.Selection(SNELL_CHART, string='LDVA (Pin Hole)', help="Left Eye Vision Using Pin Hole", readonly=True, states={'In Progress': [('readonly', False)]})
    ...
    findings = fields.One2many('sh.medical.ophthalmology.findings', 'name', string='Findings',  readonly=True, states={'In Progress': [('readonly', False)]})
    ...
    def create(self, vals):
        sequence = self.env['ir.sequence'].next_by_code('sh.medical.ophthalmology')
        vals['name'] = sequence
        return super(SHealthOphthalmology, self).create(vals)
    ...
    def unlink(self):
        ...
        return super(SHealthOphthalmology, self).unlink()
...
class SHealthOphthalmologyFindingslist(models.Model):
    _name = "sh.medical.ophthalmology.findings"
    ...
    name = fields.Many2one('sh.medical.ophthalmology', string='Evaluation', readonly=True)
