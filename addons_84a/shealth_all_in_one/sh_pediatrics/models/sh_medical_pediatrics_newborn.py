...
class SHealthPediatricsNewBorn(models.Model):
    _name = "sh.medical.pediatrics.newborn"
    _inherits={
    ...
    mother = fields.Many2one('sh.medical.patient', string='Mother', domain=[('sex','=','Female')], required=True, readonly=True, states={'Draft': [('readonly', False)]})
    ...
    apgar_scores = fields.One2many('sh.medical.pediatrics.neonatal.apgar', 'name', 'APGAR scores', readonly=True, states={'Draft': [('readonly', False)]})
    meconium = fields.Boolean(string='Meconium', readonly=True, states={'Draft': [('readonly', False)]})
    congenital_diseases = fields.Many2one('sh.medical.pathology', string='Congenital Diseases', help="Choose a disease for this medicament from the disease list. It can be an existing disease of the patient or a prophylactic.", states={'Draft': [('readonly', False)]})
    ...
    medication = fields.Many2one('sh.medical.medicines', string='Medicines', help="Prescribed Medicines", domain=[('medicament_type','=','Medicine')], readonly=True, states={'Draft': [('readonly', False)]})
    doctor = fields.Many2one('sh.medical.physician', domain=[('is_pharmacist','=',False)], string='Doctor in charge', readonly=True, states={'Draft': [('readonly', False)]}, default=_get_physician)
    signed_by = fields.Many2one('sh.medical.physician', domain=[('is_pharmacist','=',False)], string='Signed by', readonly=True, states={'Draft': [('readonly', False)]})
    ...
    cause_of_death = fields.Many2one('sh.medical.pathology', string='Cause of Death', readonly=True, states={'Draft': [('readonly', False)]})
    institution = fields.Many2one('sh.medical.health.center', string='Birth at Health Center', required=True, readonly=True, states={'Draft': [('readonly', False)]})
    state = fields.Selection(PEDIA_STATUS, string='State', readonly=True, default=lambda *a: 'Draft')

    @api.model
    def create(self, vals):
        sequence = self.env['ir.sequence'].next_by_code('sh.medical.pediatrics.newborn')
        vals['newborn_code'] = sequence
        vals['is_baby'] = True
        return super(SHealthPediatricsNewBorn, self).create(vals)
    ...
    @api.multi
    def unlink(self):
        for newborn in self.filtered(lambda newborn: newborn.state not in ['Draft']):
            raise UserError(_('You can not delete a record that is already been signed !!'))
        return super(SHealthPediatricsNewBorn, self).unlink()


class SHealthPediatricsAPGAR(models.Model):
    _name = "sh.medical.pediatrics.neonatal.apgar"
    ...
    name = fields.Many2one('sh.medical.pediatrics.newborn', string='Newborn ID')
