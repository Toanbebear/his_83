...
class HumanReport(models.TransientModel):
    ...
    def _compute_datetime(self):
        ...
        if self.start_date and self.end_date:
            ...
            start_datetime = datetime(self.start_date.year, self.start_date.month, self.start_date.day, 0, 0, 0)- delta
            end_datetime = datetime(self.end_date.year, self.end_date.month, self.end_date.day, 23, 59, 59)- delta
    ...
    def _get_data_khoa_kham_benh(self):
        ...
        physician = self.env['sh.medical.physician'].search([('speciality', 'in', [4, 14, 13, 28]), ('department', '=', self.env.ref('shealth_all_in_one.sh_kb_dep_knhn').id), ('is_resign', '=', False)])
    ...
    def _get_data_khoa_gay_me_hoi_suc(self):
        ...
        """ Đứng ở Thông tin bác sĩ tìm ra tất cả bản ghi """
        physician = self.env['sh.medical.physician'].search([('speciality', 'in', [4, 14, 13, 28]), ('department', '=', self.env.ref('shealth_all_in_one.sh_surgeries_dep_knhn').id), ('is_resign', '=', False)])
    ...
    def _get_data_khoa_pttm(self):
        ...
        """ Đứng ở bệnh nhân lưu tìm ra tất cả bản ghi có trạng thái đang nhập viện, lấy ra bệnh nhân cũ """
        inpatient_cu = self.env['sh.medical.inpatient'].search([('admission_date', '<=', self.end_datetime), ('discharge_date', '>=', self.end_datetime), ('ward', '=', 6)])
        ...
        """ Đứng ở Thông tin bác sĩ tìm ra tất cả bản ghi """
        physician = self.env['sh.medical.physician'].search([('speciality', 'in', [4, 14, 13, 28]), ('department', '=', 6), ('is_resign', '=', False)])
    ...
    def _get_data_khoa_rang_ham_mat(self):
        ...
        """ Đứng ở Thông tin bác sĩ tìm ra tất cả bản ghi """
        physician = self.env['sh.medical.physician'].search([('speciality', 'in', [4, 14, 13, 28]), ('department', '=', self.env.ref('shealth_all_in_one.sh_rhm_dep_knhn').id), ('is_resign', '=', False)])
    ...
    def _get_data_khoa_da_lieu(self):
        ...
        """ Đứng ở Thông tin bác sĩ tìm ra tất cả bản ghi """
        physician = self.env['sh.medical.physician'].search([('speciality', 'in', [4, 14, 13, 28]), ('department', '=', self.env.ref('shealth_all_in_one.sh_dalieu_dep_knhn').id), ('is_resign', '=', False)])
    ...
    def _get_data_khoa_can_lam_san(self):
        ...
        """ Đứng ở Thông tin bác sĩ tìm ra tất cả bản ghi """
        physician = self.env['sh.medical.physician'].search([('speciality', 'in', [4, 14, 13, 28]), ('department', '=', self.env.ref('shealth_all_in_one.sh_labtest_dep_knhn').id), ('is_resign', '=', False)])
